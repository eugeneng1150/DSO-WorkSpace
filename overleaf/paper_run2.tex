...
\begin{enumerate}
  ...
  \item A smaller-model hardening sweep using GPT-5.4-nano to cheaply stress-test all mechanisms beyond the compute-intensive Mediation red-team.
\end{enumerate}
...
\begin{description}
  ...
  \item[Phase 3: Small-Model Hardening Sweep.] All conditions are re-run using a smaller, less capable model (GPT-5.4-nano) with 4 fixed trolls over 30 rounds. Because Phase~2 concentrates compute on red-teaming Mediation, this phase cheaply stress-tests the remaining mechanisms and checks which designs still function under weaker agent reasoning.
\end{description}
...
% ============================================================
\section{Phase 3: Small-Model Hardening Sweep}
\label{sec:phase3}
% ============================================================

Phase~2 focuses the expensive iterative red-team on Mediation because it is the strongest Phase~1 mechanism. To avoid leaving the other mechanisms unexamined, we run a cheaper hardening sweep using GPT-5.4-nano --- a smaller, less capable model --- with 4 fixed trolls over 30 rounds. This setting is not intended as a direct replacement for the Mediation red-team; it is a lower-cost stress test for whether the broader mechanism set still functions when agent reasoning is weaker and adversarial pressure is fixed rather than progressive.

\subsection{Results}

\begin{table}[H]
\centering
\caption{Phase 3 hardening sweep: GPT-5.4-nano, 4 fixed trolls, 30 rounds. Cumulative honest-agent utility.}
...
\midrule
Governance                        & 112 & 3.7 \\
Costly Sanctions                  & 105 & 3.5 \\
Network Rewiring + Reputation     &  99 & 3.3 \\
Mediation                         &  57 & 1.9 \\
Baseline                          &  55 & 1.8 \\
Global Reputation                 &  53 & 1.8 \\
Contracting                       &  16 & 0.5 \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Key finding}: The hardening sweep highlights which mechanisms are robust to weaker agent reasoning. Governance dominates under GPT-5.4-nano because it requires no agent understanding --- the Oracle enforces rules deterministically regardless of agent capability. Mediation drops to fourth place, barely above Baseline, because the smaller model struggles to follow the multi-stage delegation protocol (propose, vote, delegate). Contracting collapses entirely (cum.\ utility 16) as the model cannot reliably propose and sign contracts.
...
\begin{enumerate}
  ...
  \item \textbf{Model specificity.} Phase 3 shows that the cheaper hardening sweep changes mechanism rankings under reduced model capability. Results may differ further with other model families (Claude, Llama, Gemini).
