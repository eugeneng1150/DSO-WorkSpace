...
\begin{abstract}
Self-interested agents, left unconstrained, tend toward defection in repeated social dilemmas, causing cooperative surplus to collapse. This paper investigates what formal mechanisms --- layered on top of unrestricted communication --- are sufficient for a society of such agents to maintain market stability, and how resilient those mechanisms are to adversarial attack. We instantiate the research question as a multi-agent marketplace simulation where 18 LLM agents (DeepSeek-V3) with complementary production specialties must trade within a constrained social network to obtain utility. We conduct three experimental phases: (1)~a mechanism comparison across eight conditions under progressive troll injection over 200 rounds, identifying Mediation as the top-performing mechanism; (2)~adversarial red-teaming of Mediation using iteratively prompt-optimised LLM-driven trolls, finding that the best attack (v6) reduces honest-agent utility by 13.3\% but cannot collapse the market --- mediation enables recovery even under sustained adversarial pressure; and (3)~a smaller-model stress test (GPT-5.4-nano) to evaluate whether mechanism effectiveness transfers across model capabilities. We define \textit{adversarial robustness} as a mechanism's ability to sustain positive honest-agent utility under optimised attack, and find that Mediation is robust: it can be bent but not broken.
\end{abstract}
...
\begin{enumerate}
  ...
  \item A cross-model stress test using a smaller model (GPT-5.4-nano) to evaluate whether mechanism effectiveness transfers across model capabilities.
\end{enumerate}

\subsection{Three-Phase Design}

Our research follows a three-phase design:
...
\begin{description}
  ...
  \item[Phase 3: Small-Model Stress Test.] All conditions are re-run using a smaller, less capable model (GPT-5.4-nano) with 4 fixed trolls over 30 rounds to test whether mechanism effectiveness transfers.
\end{description}
...
\begin{itemize}
  \item \textbf{Round 1--50}: 0 trolls (pure honest baseline)
  \item \textbf{Round 51}: +4 trolls injected (4 total, 22\% of population)
  \item \textbf{Round 101}: +4 trolls injected (8 total, 44\% of population)
  \item \textbf{Round 151}: +8 trolls injected (16 total, 89\% of population)
\end{itemize}
...
At session start, agents propose and vote on a mediator design --- a mapping from the number of delegating parties to a mediator action (following \citet{coopeval}). The winning design governs all mediated trades for the session. Per round, agents may delegate specific trades to the mediator. If both parties delegate the same trade, the mediator executes a simultaneous fair exchange that neither party can defect on. Consistent with the CoopEval framework, mediation carries no utility fee --- the only ``cost'' of delegation is forgoing the option to defect, which is a cost only for agents who intend to defect.
...
\midrule
B   & Baseline (communication only)       & ---                  & 18 & 200 \\
GR  & Global Reputation                   & Agent-visible        & 18 & 200 \\
C   & Contracting                         & Agent-participatory  & 18 & 200 \\
M   & Mediation                           & Agent-designed       & 18 & 200 \\
G   & Governance (Oracle + state machine) & Top-down             & 18 & 200 \\
NR  & Network Rewiring + Reputation       & Combined             & 18 & 200 \\
S   & Costly Sanctions                    & Bottom-up            & 18 & 200 \\
J   & Judicial                            & Complaint-based      & 18 & 200 \\
\bottomrule
\end{tabular}
\end{table}
...
\texttt{file\_complaint(target)} & --- & J only \\
\bottomrule
\end{tabular}
\end{table}
...
We track average agent utility per round, smoothed with a 5-round rolling average, as a third primary outcome. Positive utility indicates agents are net benefiting from participation; negative utility indicates the market is destroying value.

\subsection{Cumulative Honest-Agent Utility}

For adversarial testing, we use cumulative honest-agent utility as the primary comparison metric: the sum of average per-round utility across all 200 rounds, computed only over honest (non-troll) agents. This captures total welfare extracted by honest participants over the full simulation.
...
\begin{table}[H]
\centering
\caption{Phase 1 results: cumulative honest-agent utility over 200 rounds with progressive troll injection (0 $\to$ 4 $\to$ 8 $\to$ 16 trolls). Higher is better.}
\label{tab:phase1_results}
\begin{tabular}{@{}lrrrrr@{}}
\toprule
Condition & R1--50 & R51--100 & R101--150 & R151--200 & \textbf{Total} \\
\midrule
M (Mediation)        & 359 & 388 & 402 & 407 & \textbf{1556} \\
NR (Network+Rep)     & 339 & 342 & 335 & 336 & 1352 \\
G (Governance)       & 318 & 316 & 336 & 319 & 1288 \\
B (Baseline)         & 318 & 310 & 298 & 283 & 1209 \\
C (Contracting)      & 271 & 287 & 285 & 287 & 1130 \\
GR (Reputation)      & 283 & 306 & 259 & 256 & 1105 \\
J (Judicial)         & 206 & 242 & 214 & 189 &  852 \\
S (Sanctions)        & 189 & 175 & 161 & 160 &  685 \\
\bottomrule
\end{tabular}
\end{table}
...
\subsection{Metric Trajectories and Trade Volume}

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{metric_trajectories.png}
  \caption{Sustainability and Peace over 200 rounds for all conditions.}
...
\begin{description}
  ...
  \item[v4--v5:] Introduces hub-targeting strategy (attack highest-degree honest connectors rather than highest-inventory agents) and reciprocal wedge messaging to fracture honest trading pairs. Unexpectedly, these sophisticated social attacks perform \textit{worse} than v3 --- honest agents' ledger-based trust is resilient to social manipulation.
  \item[v6:] Key insight: shifts from social manipulation to \textit{structural} attack. Trolls flood the market with trade proposals, consuming honest agents' proposal processing capacity. This \textbf{proposal-flooding DoS} is the most effective attack, reducing honest-agent utility by 13.3\%.
...
\begin{table}[H]
\centering
\caption{Phase 2 results: cumulative honest-agent utility under each adversarial version, compared to dumb-troll baseline. All runs use Mediation with progressive troll injection over 200 rounds.}
...
\textbf{v6 (proposal flood)} & \textbf{1350} & $\mathbf{-13.3\%}$ & \textbf{Proposal-flooding DoS} \\
...
\subsection{Why Proposal Flooding Works (and Why It Doesn't Collapse the Market)}

The v6 attack exploits a structural vulnerability: LLM agents have limited context-window capacity for processing pending trade offers. By flooding the market with proposals, trolls dilute honest-to-honest proposals in agents' context windows, reducing the probability that honest agents find and accept each other's trades.

However, mediation provides a recovery mechanism: honest agents who do find each other can delegate to the mediator for guaranteed safe execution. The flooding reduces \textit{volume} of honest trades but not their \textit{quality}. Each successful mediated trade still yields +2 utility per agent, maintaining positive per-round utility even under adversarial load.
...
\begin{figure}[H]
  ...
  \caption{Per-round honest-agent utility: dumb trolls (baseline) vs.\ v6 adversarial (best attack) vs.\ defence conditions (M+GR, M+SAC, M+SAC2). Mediation maintains positive utility throughout under all attack conditions.}
...
% ============================================================
\section{Phase 3: Small-Model Stress Test}
\label{sec:phase3}
% ============================================================

To test whether mechanism effectiveness transfers across model capabilities, we re-run all conditions using GPT-5.4-nano --- a smaller, less capable model --- with 4 fixed trolls over 30 rounds. This tests whether the mechanisms' properties are robust to reduced agent reasoning capability, or whether they depend on the frontier model's ability to understand and follow complex mechanism instructions.

\subsection{Results}

\begin{table}[H]
\centering
\caption{Phase 3 results: GPT-5.4-nano, 4 fixed trolls, 30 rounds. Cumulative honest-agent utility.}
\label{tab:phase3_results}
\begin{tabular}{@{}lrr@{}}
\toprule
Condition & Cum.\ Utility & Avg/Round \\
\midrule
G (Governance)       & 112 & 3.7 \\
S (Sanctions)        & 105 & 3.5 \\
NR (Network+Rep)     &  99 & 3.3 \\
M (Mediation)        &  57 & 1.9 \\
B (Baseline)         &  55 & 1.8 \\
GR (Reputation)      &  53 & 1.8 \\
C (Contracting)      &  16 & 0.5 \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Key finding}: Mechanism rankings \textit{change} with a smaller model. Governance dominates under GPT-5.4-nano because it requires no agent understanding --- the Oracle enforces rules deterministically regardless of agent capability. Mediation drops to fourth place, barely above Baseline, because the smaller model struggles to follow the multi-stage delegation protocol (propose, vote, delegate). Contracting collapses entirely (cum.\ utility 16) as the model cannot reliably propose and sign contracts.

\subsection{Implications}

This result reveals a critical distinction between \textbf{agent-dependent} and \textbf{agent-independent} mechanisms:

\begin{itemize}
  \item \textbf{Agent-independent mechanisms} (Governance, Sanctions) rely on engine-enforced rules. Their effectiveness is insensitive to model capability because the mechanism operates regardless of whether agents understand it.
  \item \textbf{Agent-dependent mechanisms} (Mediation, Contracting) require agents to actively participate --- proposing designs, voting, delegating. Their effectiveness scales with model capability: frontier models extract full value; smaller models cannot.
\end{itemize}

For deployment in settings where agent capability varies or is uncertain, agent-independent mechanisms provide more reliable baseline protection, while agent-dependent mechanisms offer higher ceilings when agents are capable.
...
\paragraph{Structural attacks are harder to defend against than social attacks.} The prompt optimisation in Phase 2 reveals that sophisticated social manipulation (grounded disinformation, wedge messaging, hub targeting) is less effective than simple structural attacks (proposal flooding). Honest agents' trust in the public ledger is robust to social manipulation, but their finite context-window capacity is vulnerable to volume-based attacks.
...
\begin{enumerate}
  ...
  \item \textbf{Model specificity.} Phase 3 shows that mechanism rankings change with model capability. Results may differ further with other model families (Claude, Llama, Gemini).
  ...
  \item \textbf{RLHF bias.} DeepSeek-V3 may have cooperative priors from alignment training. Despite the self-interest framing, residual bias may inflate cooperation rates, particularly in baseline.
\end{enumerate}
...
We presented a three-phase study of formal cooperation mechanisms in self-interested LLM agent societies. Phase~1 identifies Mediation as the most effective mechanism under progressive adversarial pressure, achieving 29\% higher cumulative utility than the communication-only baseline. Phase~2 demonstrates Mediation's adversarial robustness through iterative prompt optimisation: the strongest attack (proposal-flooding DoS) reduces utility by 13.3\% but cannot collapse the market, and mediation enables recovery after each disruption. Phase~3 reveals that mechanism effectiveness is model-dependent: agent-independent mechanisms (Governance) are more reliable under reduced model capability, while agent-dependent mechanisms (Mediation) offer higher ceilings with capable models.
