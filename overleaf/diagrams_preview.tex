...
\node[bubble right, anchor=north east] (m3) at (10.8, 7.8) {
    \textbf{Agent 7} \textit{(internal reasoning):} Agent 3 has a trust score of 0.92. They delivered in our last 3 trades. I'll accept and deliver my goods honestly.
};
\node[timestamp, below right=1pt and 0pt of m3.south west] {Round 42, Trade};

% System: Resolution
\node[system] (sys3) at (5.5, 5.8) {Trade executed: Agent 3 delivers 2$\times$A $\rightarrow$ Agent 7. Agent 7 delivers 2$\times$B $\rightarrow$ Agent 3.};

% Agent 3 post-trade (left-aligned)
\node[bubble left, anchor=north west] (m4) at (0.2, 5) {
...
% System: Utility
\node[system] (sys4) at (5.5, 3.2) {Consumption: Agent 3 consumes 2$\times$B (+6 utility). Agent 7 consumes 2$\times$A (+6 utility).};

% Outcome summary bar
\node[draw, rounded corners=6pt, fill=produce!10, minimum width=10cm, minimum height=1cm, font=\small, align=center, thick] at (5.5, 1.8)
    {\textbf{Outcome:} Mutual cooperation. Both agents gain +6 utility this round.};

% Alternative outcomes note
\node[font=\scriptsize, text width=10cm, align=center, gray] at (5.5, 0.5)
    {Alternative outcomes: \textbf{Defect} (one takes goods, delivers nothing) $\mid$ \textbf{Reject} (no exchange) $\mid$ \textbf{Delegate} (mediator executes)};
