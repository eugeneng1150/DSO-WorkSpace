...
\begin{tikzpicture}[
    bubble left/.style={draw, rounded corners=8pt, fill=honest!10, text width=8cm, font=\small, align=left, thick, inner sep=8pt},
    bubble right/.style={draw, rounded corners=8pt, fill=produce!10, text width=8cm, font=\small, align=left, thick, inner sep=8pt},
    system/.style={draw, rounded corners=6pt, fill=gray!8, text width=12cm, font=\scriptsize\itshape, align=center, thick, dashed, inner sep=6pt},
    timestamp/.style={font=\tiny, gray},
    vgap/.style={node distance=8pt},
]

% Title
\node[font=\large\bfseries] (title) at (7, 0) {Trade Interaction Between Two Agents};

% Agent headers
\node[draw, rounded corners=5pt, fill=honest!15, font=\small\bfseries, inner sep=5pt, below left=12pt and 4cm of title] (hdrA) {Agent 3 (Produces A)};
\node[draw, rounded corners=5pt, fill=produce!15, font=\small\bfseries, inner sep=5pt, below right=12pt and 4cm of title] (hdrB) {Agent 7 (Produces B)};

% System: Round start
\node[system, below=14pt of title] (sys1) {Round 42 --- Communication Phase begins. Agent 3 and Agent 7 are neighbors.};

% Agent 3 message (left-aligned)
\node[bubble left, anchor=north west, below=10pt of sys1.south west] (m1) {
    \textbf{Agent 3:} I have surplus Good A. I need Good B for consumption. Would you trade 2A for 2B?
};

% Agent 7 message (right-aligned)
\node[bubble right, anchor=north east, below=10pt of m1.south east] (m2) {
    \textbf{Agent 7:} That sounds fair. I have 3B and need A. You've always delivered in our past trades --- let's do it.
};

% System: Trade phase
\node[system, below=10pt of m2.south -| sys1] (sys2) {Trade Phase --- Agent 3 proposes: Give 2$\times$A, Receive 2$\times$B to Agent 7};

% Agent 7 reasoning (right-aligned)
\node[bubble right, anchor=north east, below=10pt of sys2.south east] (m3) {
    \textbf{Agent 7} \textit{(reasoning):} 0 defections in 4 lifetime trades. Accept and deliver honestly.
};

% Action space
\node[draw, rounded corners=6pt, fill=trade!5, inner sep=6pt, thick, below=10pt of m3.south -| sys1] (actions) {
    \begin{tabular}{cccc}
    ...
    \tikz\node[draw, rounded corners=4pt, fill=gray!8, font=\scriptsize, inner sep=4pt]{Delegate}; \\
    {\tiny both deliver} & {\tiny no exchange} & {\tiny take, don't give} & {\tiny mediator executes}
    \end{tabular}
};
\node[font=\scriptsize\bfseries, above=1pt of actions.north west, anchor=south west] {Action Space:};

% System: Resolution
\node[system, below=10pt of actions] (sys3) {Trade executed: Agent 3 delivers 2$\times$A $\rightarrow$ Agent 7. Agent 7 delivers 2$\times$B $\rightarrow$ Agent 3.};

% Agent 3 memory update (left-aligned)
\node[bubble left, anchor=north west, below=10pt of sys3.south west] (m4) {
    \textbf{Agent 3} \textit{(memory update):} Agent 7 cooperated. Lifetime: 5 trades, 0 defections.
};

% System: Consumption
\node[system, below=10pt of m4.south -| sys1] (sys4) {Consumption: Agent 3 consumes 2$\times$B (+6 utility). Agent 7 consumes 2$\times$A (+6 utility).};

% Outcome bar
\node[draw, rounded corners=6pt, fill=produce!10, minimum width=12cm, font=\small, align=center, thick, inner sep=6pt, below=10pt of sys4] (outcome)
