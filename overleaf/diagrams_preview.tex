...
\begin{tikzpicture}[
    bubble left/.style={draw, rounded corners=8pt, fill=honest!10, text width=7cm, font=\footnotesize, align=left, thick, inner sep=6pt, anchor=north},
    bubble right/.style={draw, rounded corners=8pt, fill=produce!10, text width=7cm, font=\footnotesize, align=left, thick, inner sep=6pt, anchor=north},
    sysbox/.style={draw, rounded corners=6pt, fill=gray!8, font=\footnotesize\itshape, align=center, thick, dashed, inner sep=5pt, anchor=north, text width=11cm},
    phaselabel/.style={font=\scriptsize\bfseries\sffamily, fill=white, inner sep=2pt},
]
...
% Title
\node[font=\large\bfseries, anchor=north] (title) at (\cx, 0) {Trade Interaction Between Two Agents};

% Agent headers side by side
\node[draw, rounded corners=5pt, fill=honest!15, font=\footnotesize\bfseries, inner sep=4pt, anchor=north] (hdrA) at (\lx+1.5, -0.7) {Agent 3 (Produces A)};
\node[draw, rounded corners=5pt, fill=produce!15, font=\footnotesize\bfseries, inner sep=4pt, anchor=north] (hdrB) at (\rx-1.5, -0.7) {Agent 7 (Produces B)};

% ── Communication Phase ──
\node[phaselabel] at (-0.2, -1.5) {Communication};
\draw[gray!40, thick] (-0.2, -1.7) -- (12.2, -1.7);

\node[sysbox] (sys1) at (\cx, -2.0) {Agent 3 and Agent 7 are neighbors.};

\node[bubble left] (m1) at (\lx, -2.9) {
    \textbf{Agent 3:} I have surplus Good A and need Good B. Trade 2A for 2B?
};

\node[bubble right] (m2) at (\rx, -4.4) {
    \textbf{Agent 7:} Fair deal. You've always delivered. Let's do it.
};

% ── Trade Phase ──
\node[phaselabel] at (-0.2, -5.7) {Trade};
\draw[gray!40, thick] (-0.2, -5.9) -- (12.2, -5.9);

\node[sysbox] (sys2) at (\cx, -6.2) {Agent 3 proposes: Give 2$\times$A, Receive 2$\times$B};

\node[bubble right] (m3) at (\rx, -7.1) {
    \textbf{Agent 7} \textit{(reasoning):} 0 defections in 4 lifetime trades. Accept.
};

% Action space
\node[draw, rounded corners=6pt, fill=trade!5, inner sep=5pt, thick, anchor=north] (actions) at (\cx, -8.4) {
    \begin{tabular}{ccc}
    \tikz\node[draw, rounded corners=4pt, fill=produce!20, font=\scriptsize\bfseries, inner sep=3pt, thick]{Accept \checkmark}; &
    \tikz\node[draw, rounded corners=4pt, fill=gray!8, font=\scriptsize, inner sep=3pt]{Reject}; &
    \tikz\node[draw, rounded corners=4pt, fill=gray!8, font=\scriptsize, inner sep=3pt]{Defect}; \\
    {\tiny both deliver} & {\tiny no exchange} & {\tiny take, don't give} \\[2pt]
    \multicolumn{3}{c}{\tiny\itshape + mechanism-specific actions (delegate, contract, sanction, \ldots)}
    \end{tabular}
};
\node[font=\scriptsize\bfseries, above=1pt of actions.north west, anchor=south west] {Decision Policy:};

% ── Execution ──
\node[phaselabel] at (-0.2, -10.2) {Execution};
\draw[gray!40, thick] (-0.2, -10.4) -- (12.2, -10.4);

\node[sysbox] (sys3) at (\cx, -10.7) {Agent 3 delivers 2$\times$A $\rightarrow$ Agent 7. \quad Agent 7 delivers 2$\times$B $\rightarrow$ Agent 3.};

% Memory update
\node[bubble left] (m4) at (\lx, -11.6) {
    \textbf{Agent 3} \textit{(memory):} Agent 7 cooperated. 5 trades, 0 defections.
};

% ── Consumption ──
\node[phaselabel] at (-0.2, -12.9) {Consumption};
\draw[gray!40, thick] (-0.2, -13.1) -- (12.2, -13.1);

\node[sysbox] (sys4) at (\cx, -13.4) {Agent 3 consumes 2$\times$B (+6 utility). Agent 7 consumes 2$\times$A (+6 utility).};

% Outcome bar
\node[draw, rounded corners=6pt, fill=produce!10, font=\footnotesize, align=center, thick, inner sep=5pt, anchor=north, minimum width=11cm] (outcome) at (\cx, -14.3)
    {\textbf{Outcome:} Mutual cooperation. Both agents gain +6 utility.};
