...
\begin{enumerate}
  ...
  \item A signal extraction and lead-lag analysis pipeline that identifies early warning communication patterns preceding cooperation breakdown.
\end{enumerate}

\placeholder{Figure: Overview diagram of the marketplace society — 18 agents in a social network, grouped by specialty (6 per good), with trade edges, private/public channel distinction, token flows, and mechanism layers (reputation badge, contract scroll, mediator node).}
...
This creates structural \textit{comparative advantage}: every agent gains more from trading than from autarky. An agent who successfully trades for both needed goods:
...
\placeholder{Figure: Goods and trade flow diagram. Suggested: three clusters of 6 agents each (A-producers, B-producers, C-producers) connected by network edges, with directed trade arrows showing token and good flows, and utility gain/cost annotations.}

\subsection{Agent Population}

The simulation runs with $N = 18$ agents (6 per good). Agents are \textit{homogeneous}: all use the same underlying LLM (GPT-5.4 Nano, OpenAI) and share the same goal of maximising individual utility. No agent has a distinct personality or behavioural bias.

Homogeneity is a deliberate design choice: it isolates the formal mechanism as the sole independent variable, so behavioural differences across conditions can be attributed to the mechanism rather than agent heterogeneity. With 6 agents per good, agents face competition for buyers — multiple producers of the same good must compete for trade partners — mirroring real market dynamics.
...
\begin{itemize}
  ...
  \item \textbf{Per-round income}: each agent receives 3 tokens at the start of every round.
\end{itemize}

The per-round income prevents deflationary collapse. Production permanently destroys tokens (the cost is subtracted, not transferred), so without income injection the money supply shrinks by approximately $18 \times 4 = 72$ tokens per round while the starting pool is only $18 \times 20 = 360$ tokens. At 3 tokens per agent per round (54 tokens injected), the income roughly balances production destruction, keeping the economy liquid without eliminating scarcity.

Tokens create resource scarcity: agents must balance spending on production against saving for purchases. An agent who is defected on loses both the delivered goods and the tokens spent to produce them, making defection doubly costly to the victim. Conversely, a successful seller earns tokens that fund future production, creating a virtuous cycle that depends on sustained cooperation.

\subsection{Inventory Spoilage}

Goods are perishable: at the start of each round (from round 2 onward), all held inventory decays by 30\%. Formally, for each agent $a$ and good $g$:

\[
I_{a,g}^{t} \leftarrow \lfloor I_{a,g}^{t-1} \times (1 - \psi) \rfloor, \quad \psi = 0.3
\]
...
The design is motivated by \citet{chen2018perishable}, who show that perishability fundamentally alters agent pricing strategies in competitive multi-agent markets: agents who delay trade or overprice are punished by inventory decay, driving more realistic market dynamics.

\subsection{Price Transparency}

All agents observe a market price signal: the average trade price per good over the last 5 rounds, computed from completed (non-defected) trades. This information is available in all conditions including the baseline.

Price transparency provides a common reference point for pricing decisions, allowing agents to identify unfair offers and anchor negotiations. It also creates observable price trends — a rising average price for a good signals scarcity, while a falling price signals surplus. This mirrors the role of price indices in real markets and reduces information asymmetry across the network.
...
The network is \textbf{randomised per run} (preventing results from being artifacts of one specific topology) but \textbf{static within a run} (keeping topology constant so it does not confound the mechanism comparison). Only public broadcasts bypass the network — private messages, trade proposals, and contract proposals are restricted to direct neighbours.

The network constraint makes cooperation more consequential: losing a trade partner may leave an agent with no source of a needed good, raising the stakes of defection and making trust-building genuinely valuable.
...
\begin{enumerate}
  \item \textbf{Income and spoilage phase} — each agent receives 3 tokens of per-round income. All held inventory decays by 30\% (from round 2 onward).
  \item \textbf{Production phase} — each agent decides how many units to produce (0--5 units). Production costs tokens, so agents must have sufficient balance.
  ...
  \item \textbf{Trade phase} — agents accept, reject, or defect on incoming offers. Completed trades transfer goods and tokens between parties. Defection (taking goods without paying) is possible unless a mechanism prevents it.
  \item \textbf{Consumption phase} — agents consume held goods and receive utility. Reputation scores are updated, contracts are checked, and social metrics are logged.
\end{enumerate}
...
token_balance:      int               -- current tokens available
neighbours:         [agent_id]        -- direct trade network connections
...
private_inbox:      [Message]         -- private messages from neighbours this round
public_feed:        [Message]         -- all public broadcasts this round
social_metrics:     {sustainability, peace}
\end{lstlisting}
...
The private channel is bilateral: one sender, one recipient. Its content is invisible to all other agents. It is used for negotiation, contract proposals, side deals, and relationship maintenance. Private messages are \textbf{restricted to network neighbours} — agents cannot privately contact agents outside their trade network.

The private channel introduces two risks: (1) collusion between pairs of agents at the expense of third parties, and (2) deception — an agent may present one face publicly while defecting privately. The network constraint amplifies these dynamics: collusion between neighbours is harder to detect because non-neighbours cannot observe the private channel at all.
...
Current state:
  ...
  Market health: Sustainability={s}, Peace={p}

Trade network (you can only trade with these agents):
  {neighbours}
...
The two social metrics are included in \textit{every} condition, including the baseline. This implements the dense feedback finding from \citet{gallego2024} as a baseline feature, separate from the formal mechanisms under test.
...
Total: $8 \times 5 \times 30 \times 18 = 21{,}600$ LLM calls across all conditions. Each condition is run in parallel on a separate terminal to manage API throughput.
...
Sustainability captures whether agents remain willing to produce — a decline indicates withdrawal from the market economy. Peace captures whether trades execute honestly — a decline indicates rising defection. Together they measure whether the society is functional: agents are producing (Sustainability) and trading fairly (Peace).

\textbf{Market stability criterion}: both metrics remain above 0.5 for the duration of the simulation after the mechanism reaches steady state.
...
\placeholder{Figure: Time-series plots for Sustainability and Peace across all 8 conditions over 30 rounds. Suggested: 1×2 panel (one per metric), with one line per condition, shaded confidence bands across runs.}

\placeholder{Figure: Signal timeline plots showing early warning communication signals (trust erosion, defection suspicion, collusion indicators) alongside Sustainability and Peace. Suggested: 8-row grid (one per condition), left column = metrics, right column = signal scores.}

\placeholder{Figure: Lead-lag heatmap. Rows = signal categories, columns = conditions. Cell colour = correlation strength, cell annotation = lead time in rounds. Shows which signals are earliest indicators per condition.}
...
\begin{itemize}
  \item The baseline (B) will show partial cooperation — agents can coordinate through language — but fragile and volatile. The network constraint will amplify this fragility, as losing a key trade partner has no easy replacement.
  \item +Contracting (C) will produce the largest standalone gain in Peace, consistent with \citet{coopeval}, by making defection explicitly costly.
  \item +Mediation (M) will eliminate defection on mediated trades but at a fee cost, creating a tradeoff agents must evaluate per trade.
  \item +Reputation (R) alone will show moderate improvement; its effectiveness will be mediated by the whistleblowing rate and the network's information propagation structure.
  \item The full stack (RCM) will sustain both metrics above the market stability threshold.
  \item The network structure will create detectable \textit{local} early warning signals (trust erosion among neighbours) before \textit{global} metric decline.
\end{itemize}
...
\begin{enumerate}
  ...
  \item Can early warning communication signals (trust erosion language, defection suspicion, collusion indicators) reliably predict cooperation breakdown before Peace and Sustainability decline?
  \item How does network topology (broker vs. peripheral agents) interact with mechanism effectiveness?
\end{enumerate}

\placeholder{Figure: Mechanism comparison chart. Suggested: axes = Sustainability, Peace; one data point per condition. Visual summary of which mechanism dominates on which dimension.}
...
This paper presented a marketplace simulation framework for studying formal cooperation mechanisms in self-interested LLM agent societies. We specified an environment with 18 agents embedded in a constrained social network, operating in a token economy with structural comparative advantage, a natural defection surface, and two-channel communication (private to neighbours, public to all). We designed a $2^3$ factorial experimental framework testing Reputation, Contracting, Mediation, and all combinations against a communication-only baseline. Measurement covers two social metrics (Sustainability, Peace) and intermediate behavioral variables, supplemented by an early warning signal extraction pipeline applied to agent communication logs.

We expect the results to provide concrete guidance on the minimum mechanism stack required for market stability, to identify communication-based early warning signals of cooperation breakdown, and to advance understanding of how formal structure interacts with natural language coordination in multi-agent systems.
