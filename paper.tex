...
Agents operate in a closed token economy. Each agent begins with 20 tokens, which serve as the universal medium of exchange:

\begin{itemize}
  \item \textbf{Production cost}: producing one unit of a specialty good costs 1 token.
  \item \textbf{Trade payments}: when a trade completes, the buyer transfers tokens to the seller at the agreed price.
  \item \textbf{No external income}: agents earn tokens only by selling goods.
\end{itemize}
...
Agents are embedded in a static trade network that constrains who can interact directly. Each agent is connected to 4--6 neighbours, with the following guarantees:

\begin{itemize}
  \item Every agent has at least one neighbour producing each of the other two goods, ensuring the game is playable.
...
\midrule
B   & Communication only        & 18 & 30 & 10 \\
R   & +Reputation               & 18 & 30 & 10 \\
C   & +Contracting              & 18 & 30 & 10 \\
M   & +Mediation                & 18 & 30 & 10 \\
RC  & +Reputation +Contracting  & 18 & 30 & 10 \\
RM  & +Reputation +Mediation    & 18 & 30 & 10 \\
CM  & +Contracting +Mediation   & 18 & 30 & 10 \\
RCM & All three                 & 18 & 30 & 10 \\
\bottomrule
\end{tabular}
\end{table}

Total: $8 \times 10 \times 30 \times 18 = 43{,}200$ LLM calls across all conditions. Each condition is run in parallel on a separate terminal to manage API throughput.
